...
#Geräteart (Laptop, Tablet, PC,...) //ChassisSKUNumber bei Get-CimInstance Win32_ComputerSystem -Property * -> Muss vom Hersteller eingetragen werden -> nicht zuverl�ssig
...
elseIf ($a = "n") {
    ...
    If (($ErrorConnection -eq $false) -and ($Modus -eq "Remote")) {
        ...
        try { $computer = Get-CimInstance -Class CIM_ComputerSystem -CimSession $session }catch { Write-Host -ForegroundColor Red "[Fehler] Auslesen von CIM_ComputerSystem am $HostComputer" }
        try { $os = Get-CimInstance -Class CIM_OperatingSystem -CimSession $session }catch { Write-Host -ForegroundColor Red "[Fehler] Auslesen von CIM_OperatingSystem am $HostComputer" }
        try { $BIOS = Get-CimInstance -Class CIM_BIOSElement -CimSession $session }catch { Write-Host -ForegroundColor Red "[Fehler] Auslesen von CIM_BIOSElement am $HostComputer" }
        try { $cpu = Get-CimInstance -ClassName Win32_Processor -CimSession $session }catch { Write-Host -ForegroundColor Red "[Fehler] Auslesen von Win32_Processor am $HostComputer" }
        try { $gpu = Get-CimInstance Win32_VideoController -CimSession $session }catch { Write-Host -ForegroundColor Red "[Fehler] Auslesen von Win32_VideoController am $HostComputer" }
        ...
        $ReleaseID = (Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\" -Name ReleaseID).ReleaseId #Remote 
        $CurrentBuild = (Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\" -Name CurrentBuild).CurrentBuild #Remote
        ...
        try { $Monitor = Get-CimInstance -ClassName wmimonitorid -CimSession $session -Namespace root\wmi }catch { $Error_Monitor = $true }
                        
        Start-Sleep 1

        If ($Error_Monitor -eq $false) {
        ...
        $MonitorCounter = ($Monitor | measure).Count 
        ...
        elseIf ($MonitorCounter -eq 2) {
            $MonitorInfo = [PSCustomObject] @{
                ...
                Hersteller_M2  = $Hersteller2
                SN_M2          = $Seriennummer2
                YoM_2          = $YearOManufacture2
            }
        }
    ...
    elseIf(($ErrorConnection -eq $false) -and ($Modus -eq "Remote"))
    {
    try { $MAC = Get-NetAdapter -Physical -CimSession $session }catch { Write-Host -ForegroundColor Red "[Fehler] Auslesen der NIC von $HostComputer" }
    try { $computer = Get-CimInstance -Class CIM_ComputerSystem -CimSession $session }catch { Write-Host -ForegroundColor Red "[Fehler] Auslesen von CIM_ComputerSystem am $HostComputer" }
    try { $os = Get-CimInstance -Class CIM_OperatingSystem -CimSession $session }catch { Write-Host -ForegroundColor Red "[Fehler] Auslesen von CIM_OperatingSystem am $HostComputer" }
    ...
    try { $Monitor = Get-CimInstance -ClassName wmimonitorid -Namespace root\wmi }catch { $Error_Monitor = $true }
    ...
    #Software
    If (!$ErrorConnection) {
        try {Get-ItemProperty HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\* | Sort Displayname | Select-Object DisplayName, DisplayVersion, InstallDate, Publisher | Format-Table }
        catch { Write-Host Fehler Softwareinventarierung }
    }
